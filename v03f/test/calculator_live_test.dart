...
void main() {
  ...
  setUp(() {
    // Gör layouten stabil i test
  });

  testWidgets('Live-beräkning med operatorprioritet (33 + 2 × 6 = 45)',
      (tester) async {
    ...
    expect(find.descendant(of: _display(), matching: find.text('45')), findsOneWidget);
  ...
  testWidgets('Procentlogik: 12 ÷ 10 % ⇒ 120', (tester) async {
    ...
    expect(find.descendant(of: _display(), matching: find.text('120')), findsOneWidget);
  });
  ...
  testWidgets('Backspace och AC (långtryck på C)', (tester) async {
    ...
    await _tap(tester, '7');
    await _tap(tester, '8');
    expect(find.descendant(of: _display(), matching: find.text('78')), findsOneWidget);

    // C = backspace
    await _tap(tester, 'C');
    expect(find.descendant(of: _display(), matching: find.text('7')), findsOneWidget);

    // Långtryck på C = AC
    await _longPress(tester, 'C');
    expect(find.descendant(of: _display(), matching: find.text('0')), findsOneWidget);
  ...
  testWidgets('Decimal med kommatecken: 1 , 5 × 2 ⇒ 3', (tester) async {
    ...
    expect(find.descendant(of: _display(), matching: find.text('3')), findsOneWidget);
  });
  ...
  testWidgets('Delning med noll ger Error (live)', (tester) async {
    ...
    expect(find.descendant(of: _display(), matching: find.text('Error')), findsOneWidget);
  });
}